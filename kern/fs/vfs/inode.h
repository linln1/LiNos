...
struct inode_ops {
	...
	int (*vop_delete)(struct inode *node, char *path, uint32_t type);
    int (*vop_lookup)(struct inode *node, char *path, struct inode **node_store);
    int (*vop_ioctl)(struct inode *node, int op, void *data);
	int (*vop_rename)(struct inode *node, const char *old_name, const char *new_name);
...
#define vop_delete(node, name, type)								(__vop_op(node, delete)(node, name, type))
#define vop_lookup(node, path, node_store)                          (__vop_op(node, lookup)(node, path, node_store))
#define vop_rename(node, old_name, new_name)			(__vop_op(node, rename)(node, old_name, new_name))
